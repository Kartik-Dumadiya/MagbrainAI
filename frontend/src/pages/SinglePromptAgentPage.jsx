...
export default function SinglePromptAgentPage() {
  ...
  // Fetch agent info
  useEffect(() => {
    axios.get(`${process.env.VITE_API_URL || "http://localhost:3000"}/agents/${bot_id}`, { withCredentials: true })
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await axios.put(`${process.env.VITE_API_URL || "http://localhost:3000"}/agents/${bot_id}`, agent, { withCredentials: true });
